...
function groupScene:enterScene( event )
	...
	local createGroups = function()
		...
		layers:insert(layers.buttons)
                ...
                topFrame = display.newRect(layers.frame,0,0,m.w,m.h*.075)
                topFrame.anchorX = 0
                topFrame.anchorY = 0
                ...
                bottomFrame = display.newRect(layers.frame,0,0,m.bbw,m.bbh)
                ...
                title = display.newText(layers.frame, "Groups",0,0, native.systemFont, 22 )
                title.x = centerx
                ...
                leftHome = display.newText(layers.buttons, "Create", firstBox.x, firstBox.y, native.systemFont, 12 )
                ...
                rightHome = display.newText(layers.buttons, "Home", secondBox.x, secondBox.y, native.systemFont, 12 )
                rightHome:setFillColor(0,0,0)
                rightHome.x = rightHome.x - rightHome.width*.5
                rightHome.y = rightHome.y - rightHome.height*.5
                rightHome.isVisible = true
	end

        -- Create Group
	switchFirst = function()
            if(touchesAllowed == false) then return true end
                createGroup()
                --destroy()
	...
	switchSecond = function()
            if(touchesAllowed == false) then return true end
                --if event.phase == "began" then
                    --destroy()
                    storyboard.gotoScene( "menu", "slideLeft", "100")
                --return true 
                --end
	end  
